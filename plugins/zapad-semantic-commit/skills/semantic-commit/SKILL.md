---
name: zapad-semantic-commit
description: Create semantic git commits following the Conventional Commits specification, in English, as the Zapad house convention. Use when committing changes, making commits, or when asked to commit.
metadata:
  version: 1.0.0
---
...
These come before the specification below — they're the part that's ours, not upstream:

- **Commit messages are always in English** — subject, body and footers — regardless of the language the conversation is happening in.
- **One commit per logical change.** Two unrelated things touched? Two commits.
- **Never `git add -A` blindly.** Stage only the files that belong to the task at hand; leave everything else alone.
...
| Mistake | Fix |
|---------|-----|
| Message in Portuguese | Write it in English — see Zapad rules above |
